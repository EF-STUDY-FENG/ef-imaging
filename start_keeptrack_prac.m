function [accu, rec, status, exception] = start_keeptrack_prac(opts)
arguments
...
try
    ...
    introImg = imread(fullfile('intro_pic', 'keeptrack.jpg'));
    ...
    for trial = 1:p.maxTri
        ...
        while ~early_exit    
            ...
            if score
                ...
                if Errors >= 2
                    Screen('FillRect', window_ptr, BlackIndex(window_ptr));
                    instr = sprintf('错误，错误次数： %d,下降一个等级', Errors);
                    DrawFormattedText(window_ptr, double(instr), 'center', 'center', WhiteIndex(window_ptr));
                    Screen('Flip', window_ptr);
                    pause(0.75)
                    p.level = p.level - 1;
                    ...
                    Screen('FillRect', window_ptr, BlackIndex(window_ptr));
                    instr = sprintf('错误，错误次数： %d', Errors);
                    DrawFormattedText(window_ptr, double(instr), 'center', 'center', WhiteIndex(window_ptr));
                    Screen('Flip', window_ptr);
                    pause(0.75)
                end
    ...
    instr = sprintf('你的正确率是： %.2f ，达到的等级为： %d/n请按Esc键退出', accu, p.level);
    DrawFormattedText(window_ptr, double(instr), 'center', 'center', WhiteIndex(window_ptr));
    Screen('Flip', window_ptr);
    while ~early_exit
        % here we should detect for a key press and release
