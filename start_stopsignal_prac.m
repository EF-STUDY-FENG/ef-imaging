function [accu, rec, status, exception] = start_stopsignal_prac(opts)
arguments
...
% ---- configure sequence ----
config = readtable(fullfile("config", "stopsignal_prac.xlsx"));
rec = config;
rec.onset = (0:2.5:2.5*(height(config)-1))';
...
try
    ...
    introImg = imread(fullfile('intro_pic', 'stopsignal.jpg'));
    ...
    for trial_order = 1:height(config)
        ...
        feedback_onset = stim_offset + timing.feedback;
        trial_end = feedback_onset + timing.iti;
        onset_timestamp = nan;
        ...
        while ~early_exit
            ...
            if timestamp < stim_onset || (timestamp < feedback_onset - 0.5 * ifi && timestamp >= stim_offset - 0.5 * ifi)
                vbl = Screen('Flip', window_ptr);
                if timestamp >= stim_offset && isnan(offset_timestamp)
                    offset_timestamp = vbl;
                end
            elseif timestamp < stim_offset - 0.5 * ifi && timestamp >= stim_onset - 0.5 * ifi
            ...
            elseif timestamp < trial_end - 0.5 * ifi && timestamp >= feedback_onset - 0.5 * ifi
                if ~resp_made
                    if this_trial.type{:} ~= "go"
                        Screen('Flip', window_ptr);
                    else
                    ...
                    if this_trial.type{:} == "go"
                        if double(strcmp(this_trial.orient{:}, resp))
                            Screen('Flip', window_ptr);
                        else
                            instr = '错了';
                            DrawFormattedText(window_ptr, double(instr), 'center', 'center', WhiteIndex(window_ptr));
