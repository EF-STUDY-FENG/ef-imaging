function [accu, rec, status, exception] = start_stroop_prac(opts)
arguments
...
% ---- configure sequence ----
config = readtable(fullfile("Stroop_config/", "Stroop_prac.xlsx"));
rec = config;
...
try
   ...
   % display welcome/instr screen and wait for a press of 's' to start
    introImg = imread(fullfile('intro_pic', 'stroop.jpg'));
