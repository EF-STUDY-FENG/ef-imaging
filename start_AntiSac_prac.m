function [accu, rec, status, exception] = start_AntiSac_prac(opts)
arguments
...
% ---- configure sequence ----
config = readtable(fullfile("AntiSac_config", "AntiSac_prac.xlsx"));
rec = config;
...
try
    ...
    Inst1 = imread('intro_pic\AntiSac1.jpg');  %%% instruction 
    Inst2 = imread('intro_pic\AntiSac2.jpg');
    tex=Screen('MakeTexture', window_ptr, Inst1);
    ...
    tex = Screen('MakeTexture', window_ptr, Inst2);
    Screen('DrawTexture', window_ptr, tex);
    Screen('Flip', window_ptr);   % show stim, return flip time
