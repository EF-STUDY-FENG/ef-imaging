function [accu, rec, status, exception] = start_colshp_prac(opts)
arguments
...
timing = struct( ...
    ...
    'tdur', 1.75, ...
    'feedback', 0.75); % trial duration
p.color = [1 0 0; 0 1 0] * 255; % red / green
p.sz = 200; %size
cuetxt = {'XZ'; 'YS'}; %'X':shape task,'Y':color task
...
try
    ...
    instr = imread('intro_pic/ColShp.JPG');
    tex = Screen('MakeTexture', window_ptr, instr);
