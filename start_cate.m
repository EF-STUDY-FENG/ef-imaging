function [accu, rec, status, exception] = start_cate(run, window_ptr, window_rect, prac)
% arguments
...
% ---- configure sequence ----
if nargin > 3 && prac == 1
    config = readtable(fullfile("config_prac", "Cate_prac.xlsx"));
else
    TaskFile = sprintf('Cate_run%d.xlsx', run);
    config = readtable(fullfile("config/Cate_config", TaskFile));
end
rec = config;
...
imageFolder = 'Cate_config';       % figure folder
...
try
    ...
    sq=imread('Instruction\Cate.jpg');
