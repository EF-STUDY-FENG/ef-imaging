function [accu, rec, status, exception] = start_spt2back_prac(opts)
arguments
...
try
    ...
    sq=imread('intro_pic/Spt2Back.JPG');  %%% instruction 
    ...
    for block = 1:p.nBlock
        ...
        for i = 1:p.nTrialPerBlock
            ...
            if  i>2 && (~ok || isempty(key)) % feedback for practice
                str = '错了';
                if isempty(key), str = '请及时按键'; end
                Screen('FrameRect', window_ptr, 255, rects, 3);
                DrawFormattedText(window_ptr, double(str), 'center', 50, WhiteIndex(window_ptr));
                vbl = Screen('Flip', window_ptr);
                Screen('FrameRect', window_ptr, 255, rects, 3);
                WaitTill(vbl+0.95);
                vbl = Screen('Flip', window_ptr, vbl+1);
            end
