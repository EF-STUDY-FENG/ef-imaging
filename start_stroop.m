function [rec, status, exception] = start_stroop(opts)
arguments
...
% ---- configure sequence ----
config = readtable(fullfile("MateStroop", "color.xlsx"));
rec = config;
...
try
   ...
   % display welcome/instr screen and wait for a press of 's' to start
    introImg = imread(fullfile('MateStroop', 'intro2.jpg'));
